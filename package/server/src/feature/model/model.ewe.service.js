...
const exeFilePath = DATA_FOLDER_PATH + '/project/assets/EWE'
let outfilepath = ''
let filepath = ''
// 处理一下Stanze用到的图表
...
const Import_Model = async (req, res) => {
  filepath = DATA_FOLDER_PATH + req.body.filepath
  const temp = req.body.filepath.split('input')
  outfilepath = DATA_FOLDER_PATH + temp[0] + 'output' + temp[1].split('_')[0]
  console.log('Import ' + temp[1])

  try {
    const output = execSync(
      `cd "${exeFilePath}"&& ConsoleApp2.exe "${filepath}" "Import" "${outfilepath}"`,
    )
