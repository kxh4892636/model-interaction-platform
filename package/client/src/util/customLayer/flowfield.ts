...
export class FlowFieldManager {
  ...
  async Prepare(gl: WebGL2RenderingContext) {
    ...
    this.particleMapBuffer = new Float32Array(
      this.maxBlockSize * this.maxBlockSize * 3,
    ).fill(0)
    ...
    this.drawShader = await loadShader_url(
      gl,
      'draw',
      '/shaders/ribbonParticle.vert',
      '/shaders/ribbonParticle.frag',
    )
  ...
  getFieldTextures(progressRate: number) {
    ...
    return [
      this.fieldSequence[Math.floor(progress)],
      this.fieldSequence[Math.ceil(progress)],
    ]
  }
  ...
  getMaskTextures(progressRate: number) {
    ...
    return [
      this.maskSequence[Math.floor(progress)],
      this.maskSequence[Math.ceil(progress)],
    ]
  }
  ...
  private TransMercator(bbox: [number, number, number, number]) {
    const min = mapboxgl.MercatorCoordinate.fromLngLat({
      lng: bbox[0],
      lat: bbox[1],
    })
    const max = mapboxgl.MercatorCoordinate.fromLngLat({
      lng: bbox[2],
      lat: bbox[3],
    })
  ...
  tickRender(gl: WebGL2RenderingContext) {
    ...
    if (this.controller!.content === 'flow field') {
      ...
      this.textureShader!.setFloat(
        gl,
        'progress',
        this.getProgressBetweenTexture(this.beginBlock),
      )
      this.textureShader!.setFloat2(
        gl,
        'viewport',
        window.innerWidth,
        window.innerHeight,
