...
import { eweFile,ForcingFunctionData,UploadFlag } from '../../../store/eweStore';
...
const MYupload = (props) => {
  ...
  const Uploadprops = {
      ...
      async onChange(info) {
          ...
          if (info.file.status === 'done') {
            ...
            {
                setForcingFunctionData(Object.values(result.EcoSim.ForcingFunction)[0].slice(1))
            }
  ...
  return(
      <Upload {...Uploadprops}>
          <Button icon={<UploadOutlined />} disabled={UPflag}>上传水质浓度文件</Button>
      </Upload>
  );
}
