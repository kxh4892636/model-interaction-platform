...
import { Badge, Button, Divider, Select, Space, message } from 'antd'
    ...
    axios({
      ...
      if (response.status === 200) {
        ...
        if (response.data.EcoPath.StanzeSelect.length > 0) {
          ...
          setStanzeTable(
            response.data.EcoPath.StanzeGroup[value].LifeStageTable,
          )
        ...
        if (Object.keys(response.data.EcoSim).length !== 0) {
          ...
          setForcingFunctionData(
            Object.values(response.data.EcoSim.ForcingFunction)[0].slice(1),
          )
          setEggProductionData(
            Object.values(response.data.EcoSim.EggProduction)[0].slice(1),
          )
        }
