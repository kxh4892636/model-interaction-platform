...
import { RunEcoSpace_Option,RunEcoSpacae_SelectOption,RunEcoSpacae_PlotMap,RunEcoSpacae_DefaultSelect,EcoSpaceTime } from '../../store';
import axios from 'axios';
import InputMap from './InputMap';
import { serverHost } from '../../../../config/global_variable';
function getGradientColors() {
  // const colors = ["#FF0000", "#FF6602", "#FCBF02", "#ADFF25", "#64FFAC","#9DFAFF"];
  const colors = ["#9DFAFF","#64FFAC","#A4FA23","#FCBF02","#FF6602","#FF0000"];
  ...
  for (let i = 0; i < colors.length - 1; i++) {
    ...
    gradientColors.push(`rgb(${startColor.r}, ${startColor.g}, ${startColor.b})`);

    for (let j = 1; j < numSteps; j++) {
      const r = Math.round(startColor.r + (endColor.r - startColor.r) * j / numSteps);
      const g = Math.round(startColor.g + (endColor.g - startColor.g) * j / numSteps);
      const b = Math.round(startColor.b + (endColor.b - startColor.b) * j / numSteps);
...
function Map(props){
      ...
      else
      {
        ...
        for (let i = 0; i < gridSize; i++) {
          for (let j = 0; j < gridSize; j++) {
            ...
            // ctx.fillStyle = color;
            ctx.fillRect((500-14*35)/2+j * cellSize, (500-14*35)/2+i * cellSize, cellSize+0.5, cellSize+0.5);
          }
        }
      }
  ...
  return <>
          <div><canvas id="Run_gridCanvas" width="500px" height="500px" style={{border:"1px solid black"}}></canvas></div>
        </>
}
function  Maps(){
  ...
  const SwitchRunEcoSpaceMap = (value,n)=>{
    axios({
      method: "post",
      baseURL: serverHost + "/api/model/RunEcoSpace_SwitchMap",
      data: { id: value ,num: n},
    }).then((response) => {
  ...
  return(
    <div style={{display:'flex',height:"61%"}} >
      ...
      <div style={{marginLeft:"150px"}}>
        ...
        <div style={{display:'flex',marginTop:"10px"}}>
          ...
          <div style={{width:"20px",height:"500px",marginLeft:"10px",background:"linear-gradient(to top, #9DFAFF,#64FFAC,#A4FA23,#FCBF02,#FF6602,#FF0000)"}}></div>
        </div>
        
        <Button onClick={drawGrids} type="primary" shape="circle" icon={<PlayCircleFilled />} size={'small'} />
        <Progress percent={progress} style={{width:"500px"}} />
      </div>
...
function Graph(){
  const SelectOptions = [
    ...
    { value: 'Relative_Fishing_Mortality', label: 'Relative_Fishing_Mortality' },
    { value: 'Relative_predation_mortasality', label: 'Relative_predation_mortasality' },
  ...
  const SwitchRunEcoSpace = (value)=>{
    axios({
        method: "post",
        baseURL: serverHost + "/api/model/RunEcoSpace_Switch",
        data: { id: value },
    }).then((response) => {
        if (response.status === 200) {
  ...
  useEffect(()=>{
    ...
    let myChart= echarts.getInstanceByDom(document.getElementById('RunEcoSpace')); //有的话就获取已有echarts实例的DOM节点。
  ...
  return (
    ...
    <div id='RunEcoSpace' style={{height:"70vh",width:"70vw"}}></div>
    </>
  )
...
export default function RunEcosSpace() {
  ...
  return (
    <Tabs defaultActiveKey="1" items={items} style={{height:"90%"}}/>
  )
}
