...
import "./index.css"
import {useStore,Basic,Diet,DietCol,FleetModal,Detritus,FisheryDiscardFate,FisheryLandCol,FisheryLand,FisheryDiscard,FisheryDiscardCol,EWEModelID,selectedEWEModelID} from "../store"
...
export default function App() {
    ...
    useEffect(() => {
      ...
      EWEID.forEach(async (el,index)=>{
        ...
        RadioArr.push(<Radio value={resdata.data+"|"+el} key={el}>{resdata.title}</Radio>)
    ...
    <div>
      <Space>
        ...
        <Radio.Group onChange={(e) => {
            ...
            }} value={EWERadiovalue} name="asdasd">
          <Space>
            {EWEModelData}
          </Space>
        </Radio.Group>
        <Button type="primary" onClick={()=>{ImportModel(EWERadiovalue.split("|")[0])}}>导入</Button>
      </Space>
    </div>
    <Divider/>
    <div>
