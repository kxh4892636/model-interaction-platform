...
function DietComposition(GroupID, Group, Diet, Detritus, singleID) {
  ...
  Diet.forEach((el, index) => {
    ...
    if (!Object.keys(el).includes("Detritus")) {
      Data.push({
        ...
        PredID: PredID,
        RecordID: recordid,
        PreyID: GroupID["Detritus"],
        Diet: 0,
        Detritus: Detritus[GroupID[el.name] - 1]["Detritus"],
      });
  ...
  type.forEach((el, index) => {
    if (el === 1 || el === 2) {
      ...
      Data.push({
        ID: singleID,
        PredID: GroupID[Detritus[index].name],
        RecordID: recordid,
        PreyID: GroupID["Detritus"],
        Diet: 0,
        Detritus: Detritus[index]["Detritus"],
      });
      // console.log(recordid)
      // 每push一次，record就加1
      recordid += 1;
...
export const ModifyDatabase = (ModifyData, singleID, Group, Fleet) => {
  return new Promise((reslove, reject) => {
    let returndata;
    let querysql = "";
    const GroupID = genID(Group);
...
export const HandleReturn = (Basic, InputFlag) => {
...
export const FlowDiagram = (prenode, link) => {
  const order = {};
  let maxnum = 0;
  // 在R语言处理的阶段，最多TL为7
  for (let i = 1; i < 8; i++) {
