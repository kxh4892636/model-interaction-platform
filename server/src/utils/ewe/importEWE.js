function HandleGroup(data) {
  ...
  data.forEach((element) => {
    ...
    if (element.Biomass !== -9999) {
      tmp.Biomass = element.Biomass;
    }
    if (element.ProdBiom !== -9999) {
      tmp.prodbiom = element.ProdBiom;
    }
    if (element.ConsBiom !== -9999) {
      tmp.consbiom = element.ConsBiom;
    }
    if (element.EcoEfficiency !== -9999) {
      tmp.ecoefficiency = element.EcoEfficiency;
    }
    if (element.ProdCons !== -9999) {
      tmp.ProdCons = element.ProdCons;
    }
...
export const query = async (connection) => {
  ...
  const DetritusSQL = await connection.query(
    `SELECT DetritusFate FROM ecopathdietcomp where PreyID=${
      GroupSQL.filter((item) => item.GroupName === "Detritus")[0].GroupID
    }`
  );
  const FleetID = await connection.query(
    "SELECT FleetID,FleetName FROM ecopathfleet order by FleetID"
  );
