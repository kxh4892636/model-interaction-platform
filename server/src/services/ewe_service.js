...
const colorpanel = ['#5470c6', '#91cc75', '#fac858', '#ee6666', '#73c0de', '#3ba272', '#fc8452', '#9a60b4', '#ea7ccc',
                    ...
                    "#363636", "#08d9d6", "#aa96da", "#ffde7d", "#f8f3d4", "#53354a", "#903749", "#e84545", "#2b2e4a",
...
const GroupPlotName12 = ['Biomass','Consumption/Biomass','Feeding Time','P/B,predation.mort,fishing.mort','Predation mortality','Prey',
                        'Catch','F(fishing mortality)','Total discards','Discad mortality','Discard survival','Langings']   
const GroupPlotName = ['Biomass','Consumption/Biomass','Mortality:total,fish,pred','Prey','Catch']                         
const FleetPlotName = ['Catch','F(fishing mortality)','Landings','Discards','Discad mortality','Discard survival']

const exeFilePath = dataFoldURL+"/project/assets/EWE"
var outfilepath = ""
var filepath = ""
//处理一下Stanze用到的图表
const StanzePlotOption=(data)=>{
    Object.keys(data).forEach((item)=>{
        let tempStanze = data[item]
        let StanzeNumber = []
        let StanzeIndividual = []
        let StanzePopulation = []
        ...
        let option = {
            ...
            series: [
                ...
                },
                {
                    name: 'Spawing',
                    type: 'line',
                    data: [[tempStanze.Spawing,0],[tempStanze.Spawing,1]],
                    'symbol': 'none',
                },
                {
                    name: 'Separartion',
                    type: 'line',
                    data: [[tempStanze.Separartion[1],0],[tempStanze.Separartion[1],1]],
                    'symbol': 'none',
                },
...
const FlowDiagram=(prenode,link)=>{
    ...
    return {"node":node.slice(1),"link":link,"categories":categories.slice(1)}
}
...
function NetWorkAnlysis(data){
    ...
    for(let i=0;i<=data.NoTL;i++){
        if(i===0){
            ...
            FromAll.Import.push(Retain_Decimals(data.FromPP.PPImport[i]+data.FromDet.DetImport[i]))
            FromAll.ConsByPred.push(Retain_Decimals(data.FromPP.PPConsByPred[i]+data.FromDet.DetConsByPred[i]))
            FromAll.Export.push(Retain_Decimals(data.FromPP.PPExport[i]+data.FromDet.DetExport[i]))
            FromAll.ToDet.push(Retain_Decimals(data.FromPP.PPToDet[i]+data.FromDet.DetToDet[i]))
            FromAll.Respiration.push(Retain_Decimals(data.FromPP.PPRespiration[i]+data.FromDet.DetRespiration[i]))
            FromAll.Throughput.push(Retain_Decimals(data.FromPP.PPThroughput[i]+data.FromDet.DetThroughput[i]))
    ...
    let FromEvery = []
    let names = ["0","Ⅰ","Ⅱ","Ⅲ","Ⅳ","Ⅴ","Ⅵ","Ⅶ","Ⅷ","Ⅸ","Ⅹ","Ⅺ","Ⅻ"]
    ...
    for(let i=1;i<=data.NoTL+1;i++){
        ...
        if(i===1){
            tmp.id=i.toString()
            tmp.title="P"
            tmp.TST=Retain_Decimals(data.FromPP.PPThroughput[i]*100/(DetThroughtputSum+PPThroughtputSum))
            ...
            tmp.id=i.toString()
            tmp.title="D"
            tmp.TST=Retain_Decimals(data.FromDet.DetThroughput[1]*100/(DetThroughtputSum+PPThroughtputSum))
            ...
            tmp.id=i.toString()
            tmp.title=names[i]
            tmp.TST=Retain_Decimals((data.FromPP.PPThroughput[i]+data.FromDet.DetThroughput[i])*100/(DetThroughtputSum+PPThroughtputSum))
            tmp.biomass=Retain_Decimals(data.BbyTL[i])
            ...
            if(i!==data.NoTL){
                tmp2.source = i.toString()
                tmp2.target = (i+1).toString()
                tmp2.predation = Retain_Decimals(data.FromPP.PPConsByPred[i] +data.FromDet.DetConsByPred[i])
                if(FromAll.Throughput[i]>0){
                    tmp2.TE = Retain_Decimals((data.FromPP.PPCA[i]+data.FromDet.DetCAD[i]+FromAll.ConsByPred[i])/FromAll.Throughput[i])
                }
                else{
                    tmp2.TE = 0
                }
    ...
    return {nodes:nodes,edges:edges,fromevery:FromEvery}
}

function SwitchEcoSim(data,stack,id,year,MeasuredFlag=false,MeasuredData=[]){
    //时间轴
...
function SwitchGroupPlotColor(data,GroupFleetColor){
    ...
    return {"Predatorsranked":data.Predatorsranked,"Preyranked":data.Preyranked,"Fleets":data.Fleets}
}
...
const Import_Model = async (req,res)=>{
    filepath = dataFoldURL+req.body.filepath
    let temp = req.body.filepath.split("input")
    outfilepath = dataFoldURL+temp[0]+"output"+temp[1].split("_")[0]
    console.log("Import "+temp[1])

    try {
        const output = execSync(`cd "${exeFilePath}"&& ConsoleApp2.exe "${filepath}" "Import" "${outfilepath}"`);
        // const outputString = output.toString();
        // console.log(outputString)
        // 处理执行命令后的结果
        let data = JSON.parse(fs.readFileSync(outfilepath+"\\"+'Import_Data.json', 'utf-8'))
        let StanzeSelect = []
        ...
        try {
            ...
            DepthData = JSON.parse(fs.readFileSync(outfilepath+"\\"+'MapDepth.json', 'utf-8'))
        } catch (err) {
        ...
        try
        {
            ...
            return {"EcoPath":data,"EcoSpaceDepthData":DepthData,"EcoSim":ESdata}
        }catch (err) {
            console.log("EcoSimInput文件不存在");
            return {"EcoPath":data,"EcoSpaceDepthData":DepthData,"EcoSim":{}}
        }
...
const RunEcoPath = async (req,res)=>{
    ...
    try {
        const output = execSync(`cd "${exeFilePath}"&& ConsoleApp2.exe "${filepath}" "EcoPath" "${outfilepath}"`);
        // 处理执行命令后的结果
        let data = JSON.parse(fs.readFileSync(outfilepath+"\\"+'EcopathOutput.json', 'utf-8'))
        const FlowDiagramD = FlowDiagram(data.PreNode,data.Link)
        const LindmanSpine = NetWorkAnlysis(data)
        return {"Basic_Estimate":data.Basic_Estimate,"FlowDiagram":FlowDiagramD,"LindmanSpine":LindmanSpine,"Mortality":data.Mortality,"MixedTrophic":data.MixedTrophicImpacts}
      } catch (error) {
        // 处理异常情况
      }
}
...
const RunEcoSim = async (req,res)=>{
    ...
    try {
        const output = execSync(`cd "${exeFilePath}"&& ConsoleApp2.exe "${filepath}" "EcoSim" "${outfilepath}"`);
        // 处理执行命令后的结果
        let data = JSON.parse(fs.readFileSync(outfilepath+"\\"+'EcoSim_Result.json', 'utf-8'))
        let RunEcoSimPlot = JSON.parse(fs.readFileSync(outfilepath+"\\"+'RunEcosim_Plot.json', 'utf-8'))
        let EcoSimGroupPlot = JSON.parse(fs.readFileSync(outfilepath+"\\"+'EcoSim_Group_Plots.json', 'utf-8'))
        let EcoSimFleetPlot = JSON.parse(fs.readFileSync(outfilepath+"\\"+'EcoSim_Fleet_Plots.json', 'utf-8'))
        ...
        data.EcoSim_Result_Fleet.forEach((el,index)=>{
            GroupFleetColor[el.Fleet_name] = colorpanel[index+data.EcoSim_Result_Group.length]
            FleetColor[el.Fleet_name] = colorpanel[index+data.EcoSim_Result_Group.length]
        })
        //RunEcoSim里面的图画
        ...
        try {
            ...
            MeasuredData = JSON.parse(fs.readFileSync(outfilepath+"\\"+'Measured.json', 'utf-8'))
            MeasuredFlag=true
            } catch (err) {
            console.log("Measured.json文件不存在");
            MeasuredFlag = false
        }
        const option = SwitchEcoSim(RunEcoSimPlot.Biomass_relative,false,"Biomass_relative",ModelParameters.StartTime,MeasuredFlag,MeasuredData)
        data['option'] = option 
        const GroupPlotOption = SwitchGroupPlot(EcoSimGroupPlot.EcoSim_Group_Plots[0],GroupFleetColor,ModelParameters.StartTime)
        const GroupPlotColor = SwitchGroupPlotColor(EcoSimGroupPlot.EcoSim_Group_Plots[0],GroupFleetColor)
        data['GroupPlot'] = {"Option":GroupPlotOption,"Color":GroupPlotColor,"GroupColor":GroupColor}
        const FleetPlotOption = SwitchFleetPlot(EcoSimFleetPlot.EcoSim_Fleet_Plots[0],GroupFleetColor,ModelParameters.StartTime)
        const FleetPlotColor = SwitchFleetPlotColor(EcoSimFleetPlot.EcoSim_Fleet_Plots[0],GroupFleetColor)
        data['FleetPlot'] = {"Option":FleetPlotOption,"Color":FleetPlotColor,"FleetColor":FleetColor}
        return data
      } catch (error) {
        // 处理异常情况
    }
...
const RunEcoSim_Switch = async (req,res)=>{
    ...
    if(req.body.id==="Biomass_relative"){
        ...
        const option = SwitchEcoSim(RunEcoSimPlot.Biomass_absolute,true,"Biomass_absolute",ModelParameters.StartTime)
        return option
    }
    else if(req.body.id==="Catch_relative"){
        const option = SwitchEcoSim(RunEcoSimPlot.Catch_relative,false,"Catch_relative",ModelParameters.StartTime)
        return option
    }
    else if(req.body.id==="Catch_absolute"){
        const option = SwitchEcoSim(RunEcoSimPlot.Catch_absolute,true,"Catch_absolute",ModelParameters.StartTime)
        return option
    }
}
...
const GroupPlot_Switch = async (req,res)=>{
    ...
    EcoSimGroupPlot.EcoSim_Group_Plots.forEach((el,index)=>{
        if(el.GroupName===req.body.name){
            selectID=index
        }
    })
    const GroupPlotOption = SwitchGroupPlot(EcoSimGroupPlot.EcoSim_Group_Plots[selectID],GroupFleetColor,ModelParameters.StartTime)

    const GroupPlotColor = SwitchGroupPlotColor(EcoSimGroupPlot.EcoSim_Group_Plots[selectID],GroupFleetColor)
    let result = {"Option":GroupPlotOption,"Color":GroupPlotColor,"GroupColor":GroupColor}
    return result
}
...
const FleetPlot_Switch = async (req,res)=>{
    ...
    EcoSimFleetPlot.EcoSim_Fleet_Plots.forEach((el,index)=>{
        if(el.FleetName===req.body.name){
            selectID=index
        }
    })
    const FleetPlotOption = SwitchFleetPlot(EcoSimFleetPlot.EcoSim_Fleet_Plots[selectID],GroupFleetColor,ModelParameters.StartTime)

    const FleetPlotColor = SwitchFleetPlotColor(EcoSimFleetPlot.EcoSim_Fleet_Plots[selectID],GroupFleetColor)
    let result = {"Option":FleetPlotOption,"Color":FleetPlotColor,"GroupColor":GroupColor}
    return result
}
...
const RunEcoSpace = async (req,res)=>{
    ...
    try {
        const output = execSync(`cd "${exeFilePath}"&& ConsoleApp2.exe "${filepath}" "EcoSpace" "${outfilepath}"`);
        // 处理执行命令后的结果
        ...
        else
        {
            ...
            return {ResultData:data,option:Option,SelectOption:SelectOption,FirstResultMap:{id:SelectOption[0].value,data:RunEcoSpace_Map["1"][0]},Time:ModelParameters.EcoSpaceTime}
        }
...
const RunEcoSpace_Switch = async (req,res)=>{
    ...
    let data = JSON.parse(fs.readFileSync(outfilepath+"\\"+'EcoSpace_Result.json', 'utf-8'))
    let RunEcoSpacePlot = JSON.parse(fs.readFileSync(outfilepath+"\\"+'RunEcoSpace_Plot.json', 'utf-8'))
    let ModelParameters = JSON.parse(fs.readFileSync(outfilepath+"\\"+'ModelParameters.json', 'utf-8'))
    if(req.body.id==="Relative_Biomass"){
        const option = SwitchEcoSpace(RunEcoSpacePlot.Relative_Biomass,data.EcoSpace_Result_Group,"Relative_Biomass",ModelParameters.StartTime)
        return option
    }
    else if(req.body.id==="Relative_Fishing_Mortality"){
        const option = SwitchEcoSpace(RunEcoSpacePlot.FishingMort,data.EcoSpace_Result_Group,"FishingMort",ModelParameters.StartTime)
        return option
    }
    else if(req.body.id==="Relative_predation_mortasality"){
        const option = SwitchEcoSpace(RunEcoSpacePlot.PredMortRate,data.EcoSpace_Result_Group,"PredMortRate",ModelParameters.StartTime)
        return option
    }
    else if(req.body.id==="Relative_Consumption"){
        const option = SwitchEcoSpace(RunEcoSpacePlot.ConsumptRate,data.EcoSpace_Result_Group,"ConsumptRate",ModelParameters.StartTime)
        return option
    }
    else if(req.body.id==="Relative_Catch"){
        const option = SwitchEcoSpace(RunEcoSpacePlot.Relative_Catch,data.EcoSpace_Result_Group,"Relative_Catch",ModelParameters.StartTime)
        return option
    }
}
...
const RunEcoSpace_SwitchMap =  async (req,res)=>{
    ...
    return {id:req.body.id,data:RunEcoSpace_Map[req.body.id][req.body.num],time:req.body.num}
}
